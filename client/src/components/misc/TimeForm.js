...
const TimeForm = ({
  ...
  const hours =
    workTimeEndsAt !== null
      ? Array.from(Array(workTimeEndsAt - 8)).map((e, i) => i + 8)
      : Array.from(Array(14)).map((e, i) => i + 8);
  ...
  register(
    { name },
    {
      validate: tableAtSubmit =>
        numberOfSelectedHours(tableAtSubmit, hours, wdays) >= minHours ||
        `Please choose at least ${minHours} hours`,
    }
  );
