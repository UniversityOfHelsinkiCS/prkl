...
import { combinedAlgo } from "../src/algorithm/algorithm";
...
describe("Algorithm", () => {
    it("with target group size of four, divides 12 people into groups of four evenly", () => {
        const groupSizes = combinedAlgo(4, Array.from(Array(12), () => createEmptyRegistration())).map(
    ...
    it("with target group size of four, divides 11 people into two groups of four and one group of three", () => {
        const groupSizes = combinedAlgo(4, Array.from(Array(11), () => createEmptyRegistration())).map(
            group => group.userIds.length
        )
    ...
    it("with target group size of four, divides 10 people into two groups of five", () => {
        const groupSizes = combinedAlgo(4, Array.from(Array(10), () => createEmptyRegistration())).map(
            group => group.userIds.length
        )
    ...
    it("with target group size of five, divides 87 people into two groups of six and 15 groups of five", () => {
        const groupSizes = combinedAlgo(5, Array.from(Array(87), () => createEmptyRegistration())).map(
            group => group.userIds.length
        )
